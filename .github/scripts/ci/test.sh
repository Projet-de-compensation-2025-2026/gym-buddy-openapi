...
if git ls-files --error-unmatch openapi/bundled.yaml >/dev/null 2>&1; then
  echo "TEST FAIL: openapi/bundled.yaml is tracked; the \$ref tree is the consumer document" >&2
  exit 1
fi
...
if ! grep -q "Gym Buddy API" "$tmpdir/bundled.yaml"; then
  echo "TEST FAIL: bundled resolve of the \$ref tree is empty" >&2
  exit 1
fi

echo "TEST OK: OpenAPI \$ref tree lints and resolves"
